...
{
	this->_vec.iov_base = (size <= 512) ? reinterpret_cast<char*>(PoolAllocator::allocate(size)) : reinterpret_cast<char*>(::malloc(size));
...
{
	this->_vec.iov_base = (data.size() <= 512) ? reinterpret_cast<char*>(PoolAllocator::allocate(data.size())) : reinterpret_cast<char*>(::malloc(data.size()));
	...
	if (_refcnt == 0)
	{
		if (this->_allocated)
		{
			if (this->_vec.iov_len <= 512)
				PoolAllocator::deallocate(this->_vec.iov_base, this->_vec.iov_len);
			else
