...
typedef std::basic_string<wchar_t> ygg_wstring;	//	g++ のヽ(｀Д´)ノボケェ！
...
namespace ygg_term {
	...
	const ygg_wstring broken_wchar = L"?";	//	２文字以上でも可、ANSI以外は不可。
...
inline 	ygg_node & operator = (const ygg_node &X);
	bool ygg_node::operator == (const ygg_node &X) const {
		return body == X.body;
	}
	bool ygg_node::operator != (const ygg_node &X) const {
		return body != X.body;
...
template <class T>
inline ygg_smart_shell<T>::~ygg_smart_shell() {
	value->dec_ref();
}
